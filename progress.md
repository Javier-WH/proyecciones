...
- 2026-05-26 — frontend/schedule-sync — Hardened the drag/drop rollback fix by preventing concurrent manual moves while a previous snapshot is queued/in flight, ensuring queued `schedule:setState` flushes are not cancelled by React effect cleanup, validating that source events still exist before applying a move, and deduplicating event snapshots before persistence. — files: `src/components/SchoolSchedule/SchoolSchedule.tsx`
...
- 2026-06-09 — backend/socket + frontend/schedule-sync — Fixed manual placements from deposit/"No Asignadas" being lost on refresh. Root cause: `schedule:setState` payload (~1.1MB, growing) exceeded socket.io default `maxHttpBufferSize` (1MB), so the server closed the connection, the ack never arrived, and the write was dropped (the missing ack also wedged `setStateInFlightRef`). Fix: `maxHttpBufferSize: 5e7` on the socket.io Server + 20s ack timeout in `dispatch`. The earlier `lastGenerationErrors`-in-hash attempt was wrong (caused a sync loop / version explosion) and was reverted. — files: `backend/src/backEnd/socket/socket.js`, `src/hooks/useScheduleSocket.ts` — commit: none
- 2026-06-10 — frontend/schedule-sync — Fixed stale "No Asignadas" errors reappearing after refresh despite manual placement. Root cause: the filter that prunes stale `lastGenerationErrors` only ran in the inbound-sync effect (triggered by broadcasts), not on initial load from `schedule:join`. Fix: moved the filter into a separate `useEffect` that runs whenever `scheduleState.lastGenerationErrors`, `scheduleState.eventData`, `subjects`, or `trimestre` changes, ensuring stale errors are always pruned on both initial load and broadcasts. — files: `src/components/SchoolSchedule/SchoolSchedule.tsx` — commit: none
- 2026-06-15 — frontend/schedule-ui — Added section hours statistics display (total, assigned, remaining) in the schedule header. Calculation filters by PNF, turno, seccion, trayecto, and trimestre. Deduplicates events by (subjectId, day, startTime) and caps assigned hours per subject to their expected maximum to prevent duplicate events from inflating the count. Design is minimal and elegant: inline text with bullet separators, no background colors or borders. — files: `src/components/SchoolSchedule/SchoolSchedule.tsx` — commit: none
