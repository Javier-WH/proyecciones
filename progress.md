...
- 2026-06-08 — frontend/schedule-staging — Persist manual placement cleanup for deposit/unassigned subjects by including filtered `lastGenerationErrors` in `schedule:setState` snapshots and in the sync hash. — files: `src/components/SchoolSchedule/SchoolSchedule.tsx`, `backend/src/backEnd/schedule/stateTypes.js` — commit: none
